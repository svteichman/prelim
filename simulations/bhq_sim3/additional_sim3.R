...
generate_dat <- function(n = 2000, p = 1000, k = 200, A = 3.5) {
  X <- matrix(rnorm(n*p),nrow = n)
  X_svd <- svd(X)
  X <- X_svd$u %*% diag(X_svd$d) %*% diag(nrow = p)
  X <- normalize(X)
  z <- rnorm(n, 0, 1) 
...
get_fdr <- function(n = 2000, p = 1000, k = 200, A = 3.5) {
  dat <- generate_dat(n, p, k, A)
  X <- dat$X
  y <- dat$y
...
bhq_sim3 <- get_fdr(A = A_seq[1])

for (i in 2:length(A_seq)) {
  new_res <- get_fdr(A = A_seq[i])
  bhq_sim3 <- rbind(bhq_sim3, new_res)
}

args <- commandArgs(trailingOnly = TRUE)
save(bhq_sim3, file = paste0("extra_sim3",args,".rda"))
