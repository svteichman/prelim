...
generate_dat <- function(n = 3000, p = 1000, k = 30, A = 3.5) {
  mean <- rep(0,p)
  cov <- diag(x = 1, nrow = p)
...
get_fdr <- function(n = 3000, p = 1000, k = 30, A = 3.5) {
...
bhq_sim2b <- get_fdr(A = A_seq[1])

for (i in 2:length(A_seq)) {
  new_res <- get_fdr(A = A_seq[i])
  bhq_sim2b <- rbind(bhq_sim2b, new_res)
}

args <- commandArgs(trailingOnly = TRUE)
save(bhq_sim2b, file = paste0("BHq_sim2b",args,".rda"))
