...
inc_err <- apply(hivdata[,x_cols], c(1:2), function(x) grepl("\\*|[a-d]|[e-h]|[j-z]", x)) 
row_err <- rowSums(inc_err) == 0
rmv <- which(row_err == FALSE)
hivdata=hivdata[-rmv,]

mutnums=read.table('genetic_application/PI/treatment_mut_table.txt')
...
save(hivX, file = "genetic_application/PI/hivX.rda")
save(hivY, file = "genetic_application/PI/hivY.rda")
save(Xnums, file = "genetic_application/PI/Xnums.rda")

# run analysis -----
run_hiv_data <- function(Xthresh,q) {
  ny <- dim(hivY)[2]
  
  tmtpos <- unique(pi_tsm[,1])
  Xpos <- Xnums[,1]
...
save(res_100, file = "genetic_application/rda_files/pi_res_100.rda")
load(file = "genetic_application/rda_files/pi_res_100.rda")

#info to add to plots 
drugnames <- names(hivdata[4:10])
...
plot_quant <- function(tmp1, tmp2, file_name) {
  ...
  ggplot(plot_res, aes(x = method, y = value,  fill = type)) + 
    ...
    facet_wrap(~drug, scales = "free") + ylim(c(0,40)) + 
    scale_fill_manual(values = c("midnightblue","chocolate2")) +
    ylab("# HIV-1 protease positions selected") + xlab("Quantiles of Knockoff Results") + 
    geom_hline(yintercept = tot_tsm, linetype = "dashed") +
    theme_bw()  +
    theme(legend.title = element_blank(), legend.position = c(0.5, 0.2),
          panel.spacing = unit(1, "lines"),
...
plot_quant(res_10_quant, res_90_quant, "pi_q10q90.png")
