...
save(hivX, file = "genetic_application/NRTI/hivX.rda")
save(hivY, file = "genetic_application/NRTI/hivY.rda")
save(Xnums, file = "genetic_application/NRTI/Xnums.rda")
...
save(res_100, file = "genetic_application/rda_files/nrti_res_100.rda")
load(file = "genetic_application/rda_files/nrti_res_100.rda")


#info to add to plots 
drugnames <- names(hivdata[4:9])
...
plot_quant <- function(tmp1, tmp2, file_name) {
  ...
  ggplot(plot_res, aes(x = method, y = value,  fill = type)) + 
    ...
    scale_fill_manual(values = c("midnightblue","chocolate2")) +
    ylab("# HIV-1 RT positions selected") + xlab("Quantiles of Knockoff Results") + 
    geom_hline(yintercept = tot_tsm, linetype = "dashed") +
    theme_bw()  +
    theme(legend.title = element_blank(), legend.position = "none",
          panel.spacing = unit(1, "lines"),
...
plot_quant(res_10_quant, res_90_quant, "nrti_q10q90.png")
