# This code is taken from the tutorial 4 from the knockoffs package. It claims to use the same analysis as in the 
# 2014 paper and has the same data source. However, many of the bar graphs produced are different than in the papaer.
# I've contacted the author/maintainer of the R package, so hopefully he will help me understand the differences. 

# Next steps are to rewrite the code for data manipulation, and to figure out how to exactly replicate the results 
# of the paper. 
...
comparisons <- lapply(results, function(drug) {
  lapply(drug, function(selected) {
    ...
    false_discoveries = length(setdiff(positions, pi_true_pos))
...
knockoff_and_bhq <- function (X, y, q) {
  ...
  # Run the knockoff filter.
  knock.gen = function(x) create.fixed(x, method='equi', y = y)
  result = knockoff.filter(X, y, fdr=fdr, knockoffs=knock.gen, statistic=stat.glmnet_lambdasmax)
  knockoff_selected = names(result$selected)
